...
import { dataService, type Group, type Event } from '../../lib/data';
import { Card } from '../../components/ui/card';
import { Button } from '../../components/ui/button';
import { Users, Calendar, ArrowRight, UserPlus, Search } from 'lucide-react';
...
export default function GroupsPage() {
  ...
  const [groupActivities, setGroupActivities] = useState<Record<string, number>>({});
  ...
  const loadData = async () => {
    ...
    try {
      ...
      // Mock upcoming activities count for each group
      const counts: Record<string, number> = {};
      all.forEach(g => {
        counts[g.id] = Math.floor(Math.random() * 5); // Mock count
      });
      setGroupActivities(counts);
    } catch (error) {
  ...
  if (isLoading) return <div className="p-10 text-center">Loading groups...</div>;
          ...
          <section className="space-y-6">
            ...
            <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-6">
              {joinedGroups.map(group => (
                <Card 
                  key={group.id} 
                  className="p-6 hover:shadow-xl transition-all cursor-pointer border-2 border-primary/5 group" 
                ...
                >
                  <div className="flex flex-col gap-4">
                    ...
                    <div className="flex items-center gap-4 pt-2 border-t text-xs text-muted-foreground">
                      ...
                      <span className="flex items-center gap-1.5 font-medium">
                        <Calendar className="h-3.5 w-3.5" /> {groupActivities[group.id] || 0} Activities
                      </span>
                    </div>
                  </div>
                </Card>
        ...
        <section className="space-y-6">
          ...
          <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-6">
            {filteredGroups.filter(g => !joinedGroups.some(jg => jg.id === g.id)).map(group => (
              <Card 
                key={group.id} 
                className="p-6 hover:shadow-xl transition-all border-2 border-primary/5 flex flex-col justify-between"
              >
                <div className="space-y-4">
                  ...
                  <div className="flex items-center gap-4 text-xs text-muted-foreground">
                    <span className="flex items-center gap-1.5 font-medium">
                      <Users className="h-3.5 w-3.5" /> 8 Members
                    </span>
                    <span className="flex items-center gap-1.5 font-medium">
                      <Calendar className="h-3.5 w-3.5" /> {groupActivities[group.id] || 0} Activities
                    </span>
